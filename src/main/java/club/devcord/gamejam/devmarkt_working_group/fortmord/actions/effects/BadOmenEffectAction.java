...
@LevelAction(value = 200, levelBound = false)
public class BadOmenEffectAction extends AbstractLevelAction {
    public BadOmenEffectAction(@Parameter Player player, @Parameter int level, FortMord fortMord) {
        super(player, level, fortMord);
    }

    @PostConstruct
    void start() {
        runTaskTimer(this::apply, 5, () -> random(120, 180) * 20);
    }

    private void apply() {
        if(random(0, 257) > descLevel()) {
            player().addPotionEffect(new PotionEffect(PotionEffectType.BAD_OMEN, 1800, 2));
        }
    }
}
